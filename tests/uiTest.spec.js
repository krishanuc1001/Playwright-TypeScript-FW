...
test("Test 1: Handle list of elements | Static Select Drop-down | Radio Button | Check box", async ({
  browser,
}) => {
  ...
  const userRadioBtn = page.locator(
    "//input[@value='user']//following-sibling::span"
  );
  const adminRadioBtn = page.locator(
    "//input[@value='admin']//following-sibling::span"
  );
  const acceptRadioBtn = page.locator("//button[@id='okayBtn']");
  const documentLink = page.locator(
    "//a[contains(@href, 'documents-request')]"
  );
...
test("Test 2: Window Handling", async ({ browser }) => {
  ...
  const newPageText = await newPage
    .locator("//p[@class='im-para red']")
    .textContent();
...
test("Learn exclusive Playwright Locators", async ({ browser }) => {
  ...
  await page.getByLabel("Gender").selectOption("Male"); // Dropdown
...
test("Handle Calendar Validations ", async ({ browser }) => {
  ...
  expect(
    await newPage.locator("//div[@class='date-field-container']").isVisible()
  ).toBeTruthy();
  ...
  await newPage
    .locator("//button[@class='react-calendar__navigation__label']")
    .click();
  await newPage
    .locator("//button[@class='react-calendar__navigation__label']")
    .click();
  ...
  await newPage
    .locator(
      "//button[contains(@class, 'react-calendar__year-view__months__month')]"
    )
    .nth(Number(month) - 1)
    .click();
...
test("Handle iframes", async ({ browser }) => {
  ...
  console.log(
    await framePage.locator("//*[@class='text']//h2//span").textContent()
  );
});
