...
test("E2E Scenario: Add to Cart", async ({ browser }) => {
  ...
  const countryOptionsDropdown = page.locator(
    "//section[contains(@class, 'ta-results')]"
  );
  const numOfCountries = await countryOptionsDropdown
    .locator("//button//i")
    .count();
  ...
  const orderIDLocator = page.locator(
  ...
  for (let i = 0; i < numOfCountries; ++i) {
    if (
      (
        await countryOptionsDropdown.locator("//button//i").nth(i).textContent()
      ).trim() === "India"
    ) {
...
test("E2E Scenario: Add to Cart | Special locators", async ({ browser }) => {
  ...
  const products = page.locator("//div[@class='card-body']");
  const productName = "IPHONE 13 PRO";
  const cartLocator = page
    .locator("//li")
    .getByRole("button", { name: "Cart" });
  const productList = page.locator("//div//li");
  const productNameLocator = page.getByText(productName);
  const countryOptionsDropdown = page.locator(
    "//section[contains(@class, 'ta-results')]"
  );
  ...
  const thankYouText = page.getByText("Thankyou for the order.");
  const orderIDLocator = page.locator(
    "//td[@class='em-spacer-1']//label[@class='ng-star-inserted']"
  );
  const rows = page.locator("//tbody//tr");
